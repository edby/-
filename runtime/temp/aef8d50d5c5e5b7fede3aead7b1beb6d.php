    ...
    <div class="main-content-inner">
      <div class="breadcrumbs" id="breadcrumbs">
        <ul class="breadcrumb">
          ...
          <li> <a href="<?php echo url('index'); ?>">轮播图管理</a> </li>
          <li class="active"><?php echo $pagename; ?></li>
        </ul>
      </div>
      <div class="page-content">
        <div class="page-header">
          <h1> <?php echo $pagename; ?> <small> <i class="ace-icon fa fa-angle-double-right"></i> 一共上传了<?php echo $list['count']; ?>张轮播图 </small></h1>
        </div>
        <!-- /.page-header -->
        <div class="row">
          <div class="col-xs-12"> 
            ...
            <div class="row">
                ...
                <form action="<?php echo url('index'); ?>" method="get" class="form-inline" role="form">
                  <div class="form-group">
                    <label>关键词：</label>
                    <input name="keywords" type="text" class="form-control search" placeholder="标题">
                  </div>&nbsp;&nbsp;
                  <div class="form-group"><label>状态：</label>
                    <select name="state" class="form-control" <!--onchange="look_state(this)"-->>
                      ...
                      <?php if(is_array($state) || $state instanceof \think\Collection || $state instanceof \think\Paginator): $i = 0; $__LIST__ = $state;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo $vo['value']; ?>" <?php if($get_state == $vo['value']): ?>selected = 'selected'<?php else: endif; ?>><?php echo $vo['key']; ?></option>
                      <?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                  </div>&nbsp;&nbsp;
                  <button type="submit" class="btn btn-sm btn-primary">查询</button>
                  <a class="btn btn-sm btn-success" style="float:right; margin-right:10px;" href="<?php echo url('add'); ?>" >添加轮播图</a>
                  ...
                  <tbody>
                    <?php if(is_array($list['list']) || $list['list'] instanceof \think\Collection || $list['list'] instanceof \think\Paginator): $k = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
                      <tr>
                        ...
                        <td class='t_img'><img src="<?php echo $vo['link']; ?>"> </td>
                        <td> <?php echo $vo['create_time']; ?> </td>
                        <td><div class='<?php echo $vo['button']; ?>' onclick="change_status(<?php echo $vo['id']; ?>,<?php echo $vo['state']; ?>)"><?php echo $vo['state_type']; ?><div></td>
                        <td>
